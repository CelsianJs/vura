...
| **CLI Integration** | `then deploy`, `then link`, `then env` | Additions to @celsian/then-cli |
...
(generated by `generateFunctionEntry()` in `@celsian/then-core/build.ts`).
...
1. **As serverless functions** — for HTTP-triggered invocation via `/__tasks/<name>`
   (using `generateTaskEntry()` from `@celsian/then-core/build.ts`)
...
### New Commands Added to @celsian/then-cli

The ThenJS CLI (`then`) gets new commands that communicate with Vura's API.
These additions live in the open-source `@celsian/then-cli` package but call the
...
The open-source ThenJS repo will include a `@celsian/then-adapter-vura` package that
...
```typescript
// @celsian/then-adapter-vura — lives in open-source ThenJS repo
...
This follows the exact `ThenAdapter` interface from `@celsian/then-core/config.ts`:
```typescript
...
Detection order:
  ...
  2. package.json has "@celsian/then-core" dependency → ThenJS
