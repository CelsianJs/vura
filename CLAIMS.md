...
| 14 | `og:url` set to `https://vura.io` | `sites/landing/index.html` (now redirected) | Canonical domain is `vura.dev`. Was wrong metadata. Removed with the redirect. Redirect retargeted to `https://github.com/CelsianJs/vura` (safe until vura.dev is the docs site — vura.dev currently serves the Vura Platform control plane). | removed from sites/landing (redirect retargets to GitHub) |
