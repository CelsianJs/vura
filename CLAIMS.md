...
| 6 | "Hot routes: no timeout, in-memory state, websockets" | (future /ladder/4-hot) | `packages/core/test/hot-routes.test.ts` (15 tests, committed on this branch); "no timeout" = no framework-imposed limit; host/platform limits still apply | verified at code level |
...
| 14 | `og:url` set to `https://vura.io` | `sites/landing/index.html` (now redirected) | Canonical domain is `vura.dev`. Was wrong metadata. Removed with the redirect. | removed from sites/landing (redirect) |
