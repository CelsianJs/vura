...
describe('cloudflare adapter clean tarball smoke', () => {
  it('resolves @celsian/then-core from flattened clean installs', async () => {
    ...
    try {
      ...
      const nestedCore = join(app, 'node_modules/@celsian/then-adapter-cloudflare/node_modules/@celsian/then-core');
...
const mod = await import(${JSON.stringify('file://' + join(app, 'node_modules/@celsian/then-adapter-cloudflare/dist/index.js'))});
